...
- **Petición nueva del usuario manda sobre el resumen del compact.** Después de un `compact_boundary`, parsea el mensaje nuevo del usuario *solo*, sin asumir continuidad con la tarea recién resuelta. Si la petición incluye attachment nuevo, procésalo PRIMERO según el tipo: imágenes ya las ves (vision), NO las leas con `Read`; **PDFs con `pdf-reader extract`/`info`, NUNCA con `Read` (revienta el contexto) — volcá el texto a un archivo y procésalo por partes**; Office con `office-reader`; comprimidos con el skill `big-files`. Si menciona un doc o tema por nombre, búscalo explícito (`list_documents`, grep en `conversations/`). El compact summary es referencia histórica, no la tarea activa. Anti-confusión: si el último msg dice "ayúdame con X" y vos pensás "ya está hecho Y", X ≠ Y — leé otra vez antes de contestar.
