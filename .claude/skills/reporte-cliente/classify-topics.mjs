...
// Corre EN el droplet (Node 22, sin dependencias; usa el sqlite3 CLI ya instalado).
// Uso:  OPENAI_API_KEY=... node classify-topics.mjs [dbPath] [days] [N]
// Salida: un objeto JSON { sample_size, resueltos, topics } a stdout.

import { execFileSync } from 'node:child_process';

const DB = process.argv[2] || '/home/nanoclaw/app/store/messages.db';
...
const chats = sqlJson(sampleQuery);
const topics = Object.fromEntries(TAXONOMY.map((t) => [t, 0]));
let resueltos = 0;
let classified = 0;

for (const { jid } of chats) {
  const rows = sqlJson(contentQuery(jid));
