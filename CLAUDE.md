...
**WABA (Formmy) channels — verified + hardened 2026-07-02 (commit `fbcb390`).** The WABA path (`src/channels/formmy-whatsapp.ts` `processMedia`) never had the ptt bug: Meta Cloud API delivers media **pre-classified by `type`**, so a forwarded audio arrives as `type:'audio'` and is saved + transcribed. Formmy's bridge (audited in `formmy_rrv7` — `webhook.tsx` dispatch + `server/channels/handler.ts:322-414` forward) **already forwards** video (`media.type:'video'`), location/contacts/reaction (top-level fields) correctly. The gaps were on the **receiver**, now fixed:
- **video** → new `case 'video'` saves to `attachments/video-<ts>.<ext>` + `[Video: …]` (before: `default` → `[Media: video]`, file not saved).
- **location / contacts / reaction** → parsed from the body and folded into `content` (`[Location: lat,lng …]` mirrors Baileys, `[Contacto: …]`, `[Reacción: …]`). **Before:** ignored → a bare location pin (`content:""`) was dropped by `if (!finalContent)` → **400 "No content after processing"**.
